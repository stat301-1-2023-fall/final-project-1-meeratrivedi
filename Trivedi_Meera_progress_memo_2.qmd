...
writers_lines <- head(writers_lines, n=9)
...
writers_episodes <- head(writers_episodes, n = 9)
...
@fig-bar5 next explores the directors and the number of episodes they have directed. 
...
So far, I have done all the univariate analyses I plan to do, but I may do more depending on what multivariate analyses I decide to do. I've organized this progress memo in a way that showcases the progress I've made in each category of analysis, but for the final report I may re-organize in a way that shows the analyses that more closely correspond to each other together. For example: I will probably put the two scatterplots I have made together since they relate to each other. Next I need to complete the multivariate analyses, and have multiple pertaining to data from each of the individual datasets. Right now I have only examined the lines, and going forward I want to examine relationships between the many other variables I have in my joined datasets. I am sure as multivariate analyses begin to form, relevant summary statistics will as well, so I will then make more data tables displaying those. I believe I have succeeded thus far at making all my analyses visually appealing, but the things I will work on for the final project are ordering them in a way that relates them to each other and exploring them through different varieties of distributions. Bargraphs have made the most sense for each analyses so far since I have mostly been showing count, but I want to use varying geom layers and find new summary statistics. I have discovered so many interesting things about this show so far and I will work to ensure my multivariate analyses not only explore more, but tie the work I've done so far together. My next step will be to work on relationships between my numerical data, probably in scatterplot form, and then go from there.
I also do need to create codebooks for my data and create a READ.me file for my plots folder. 
